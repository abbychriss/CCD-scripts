...
def adc_plotter_compare(fnames, trace=False, psd=False, V0=0, t0=0, delta_t=500):
    ...
    if trace:
        plot_trace(df_list, t0, delta_t, label_=labels)
    
    if psd:
        plot_psd(df_list, npsd=5000, nsampint=150, fs=15.E6, label_=labels)
...
def plot_psd(df_list, npsd=5000, nsampint=150, fs=15.E6, title_="Power Spectral Distribution", label_=None):
    ...
    for i, (df, label__) in enumerate(zip(df_list, label_)):
        ...
        label2__  = r"$\sigma_{ADU}$="+f"{sig_ADU:.2e}   "
        label2__ += r"$\sigma_{CDS}$="+f"{sig_CDS:.2e}"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--delta_t', type=float, default=10000, help='Time interval as a number.')
    parser.add_argument('--save_plot', action='store_true', default=False, help='Save plots as pdf to working directory (boolean)')
    ...
    if args.save_plot==True:
        if not args.compare:
            plt.savefig(args.fname[0][:-4]+'.jpg',dpi=300)
        if args.compare:
            plt.savefig(args.fname[0][0:10]+'compare.jpg',dpi=300)
