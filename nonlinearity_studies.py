...
def find_electron_peaks(data, width, buffer, 
    ...
    counts, edges = np.histogram(data, bins=min(5000, math.floor((hist_range[1]-hist_range[0])*bin_factor)),range=hist_range)
...
#---------------- (3) Fit nonlinearity ----------------------------
def fit_nonlinearity(peaks, pedestal, gain, fit_range_right, fit_bounds_low=-100, fit_bounds_high=100):
